...
const CreateUser: React.FC = () => {
  ...
  const handleAdminAuth = async () => {
    ...
    try {
      ...
      setName('');
      setPhone('');
      setIsAdmin(false); // Reseta o checkbox
      setShowModal(false); // Fecha o modal
      setAdminPassword(''); // Reseta a senha do admin
      ...
      if ((err as Error).message.includes('auth/email-already-in-use')) {
        ...
        setError('Erro ao criar usuário. Verifique as informações e tente novamente.');
      }
      setShowModal(false); // Fecha o modal
      setAdminPassword(''); // Reseta a senha do admin
    }
      ...
      <form onSubmit={handleCreateUser}>
        ...
        <div className="mb-4 relative">
          ...
          <input
            ...
            className={`py-2 border-b placeholder:text-D4D4D4 focus:outline-none w-80 md:w-96 ${passwordError ? 'border-red-500' : 'border-black'}`}
        ...
        <div className="mb-4 relative">
          ...
          <input
            ...
            className={`py-2 border-b placeholder:text-D4D4D4 focus:outline-none w-80 md:w-96 ${confirmPasswordError ? 'border-red-500' : 'border-black'}`}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white py-8 rounded-lg shadow-md font-bold text-white text-center text-base">
            <p className="text-black font-medium mb-6">Insira a senha de administrador</p>
            <input
              type="password"
              value={adminPassword}
              onChange={(e) => setAdminPassword(e.target.value)}
              className={`pb-2 mx-20 border-b placeholder:text-D4D4D4 focus:outline-none w-64 font-normal text-black placeholder:font-normal mb-8 ${error ? 'border-red-500' : 'border-black'}`}
